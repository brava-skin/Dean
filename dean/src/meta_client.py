...
class MetaClient:
    ...
    def ensure_adset(
        ...
        *,
        ...
        if self.dry_run or not USE_SDK:
            return {"id": f"AS_{abs(hash(name)) % 10_000_000}", "name": name, "mock": True, "campaign_id": campaign_id, "daily_budget": daily_budget}
        self._init_sdk_if_needed()
    ...
    def create_video_creative(
        ...
        *,
        ...
        params = {
            "name": _s(name),
            "object_story_spec": {"page_id": pid, "video_data": video_data},
        }
    ...
    def create_validation_adset(self, campaign_id: str, creative_label: str, daily_budget: float = 40.0) -> Dict[str, Any]:
        return self.ensure_adset(_s(campaign_id), f"[VALID] {_s(creative_label)}", daily_budget)

    def create_scaling_adset(self, campaign_id: str, creative_label: str, daily_budget: float = 100.0) -> Dict[str, Any]:
        return self.ensure_adset(_s(campaign_id), f"[SCALE] {_s(creative_label)}", daily_budget)
