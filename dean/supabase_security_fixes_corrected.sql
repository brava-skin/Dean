-- =====================================================
-- SUPABASE SECURITY FIXES - CORRECTED VERSION
...
-- =====================================================
-- 5. FIX VECTOR EXTENSION ISSUE (CORRECTED)
-- =====================================================
...
DO $$
BEGIN
    -- If vector extension exists in public schema, we need to handle it carefully
    IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'vector' AND extnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')) THEN
        ...
        ALTER TABLE creative_intelligence 
        ALTER COLUMN similarity_vector TYPE TEXT;
        ...
        -- Update the column back to vector type in ml_schema with proper casting
        ALTER TABLE creative_intelligence 
        ALTER COLUMN similarity_vector TYPE ml_schema.vector(384) 
        USING similarity_vector::ml_schema.vector(384);
    ELSE
        -- If vector extension doesn't exist in public, just create it in ml_schema
        CREATE EXTENSION IF NOT EXISTS "vector" SCHEMA ml_schema;
        
        -- Update the column to use ml_schema.vector if it exists
        IF EXISTS (SELECT 1 FROM information_schema.columns 
                  WHERE table_name = 'creative_intelligence' 
                  AND column_name = 'similarity_vector') THEN
            -- Check if the column is already a vector type
            IF EXISTS (SELECT 1 FROM information_schema.columns 
                      WHERE table_name = 'creative_intelligence' 
                      AND column_name = 'similarity_vector' 
                      AND data_type = 'USER-DEFINED') THEN
                -- Column is already a vector type, just update the schema reference
                ALTER TABLE creative_intelligence 
                ALTER COLUMN similarity_vector TYPE ml_schema.vector(384) 
                USING similarity_vector::ml_schema.vector(384);
            ELSE
                -- Column is not a vector type, convert it
                ALTER TABLE creative_intelligence 
                ALTER COLUMN similarity_vector TYPE ml_schema.vector(384) 
                USING similarity_vector::ml_schema.vector(384);
            END IF;
...
EXCEPTION
    WHEN OTHERS THEN
        -- If there's any error, just create the extension in ml_schema
        -- and leave the column as is for now
        CREATE EXTENSION IF NOT EXISTS "vector" SCHEMA ml_schema;
        RAISE NOTICE 'Vector extension created in ml_schema. Column type conversion skipped due to error: %', SQLERRM;
END $$;
...
-- Grant usage on ml_schema to service_role
